...
import org.firstinspires.ftc.teamcode.MultiSystem.CollectFromSubSetP;
import org.firstinspires.ftc.teamcode.MultiSystem.CollectFromSubSetPIn;
...
@TeleOp
public class CompTeleOpOneDriverMatan extends CommandOpMode {
    ...
    public Trigger gamepadLeftTrigger;

    public DoubleSupplier rightTriggerSupplier;
    public DoubleSupplier leftTriggerSupplier2;
    ...
    public void initialize() {
        ...
        leftTriggerSupplier2 = () -> gamepadEx1.getTrigger(GamepadKeys.Trigger.LEFT_TRIGGER);

        gamepad2rightTrigger = new Trigger(() -> rightTriggerSupplier.getAsDouble() > 0.1);

        gamepadLeftTrigger = new Trigger(() -> leftTriggerSupplier2.getAsDouble() > 0.1);

        gamepad2rightTrigger.whileActiveContinuous(
                new CollectFromSubSetP(elbowArm, extenderArm, clawUpDown,claw, rightTriggerSupplier)
        );

        gamepadLeftTrigger.whileActiveContinuous(
                new CollectFromSubSetPIn(elbowArm, extenderArm, clawUpDown,claw, rightTriggerSupplier)
        );

//        gamepad2rightTrigger.and(gamepadLeftTrigger).whenActive(new InstantCommand(()->extenderArm.setPower(0)));

        gamepad2rightTrigger.whenInactive(
                new InstantCommand(()->extenderArm.setPower(0))
        ).and(gamepadLeftTrigger.whenInactive(
                new InstantCommand(()->extenderArm.setPower(0))
                ));
